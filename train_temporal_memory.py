...
def load_p23_base_weights(
    ...
    if saved_memory is not None:
        ...
        saved_temporal_diff = bool(
            saved_memory.get('temporal_diff_enabled', False)
        )
        saved_ssa = bool(saved_memory.get('ssa_enabled', False))
        ...
        adding_temporal_diff = bool(temporal_diff_enabled) and not saved_temporal_diff
        adding_ssa = bool(ssa_enabled) and not saved_ssa
        if saved_temporal_diff and not bool(temporal_diff_enabled):
            raise ValueError(
                'M22 temporal-diff branch is present in the checkpoint but disabled '
                'in the configured architecture.'
            )
        if saved_ssa and not bool(ssa_enabled):
            raise ValueError(
                'M22 SSA branch is present in the checkpoint but disabled in the '
        ...
        if (
            ...
            if adding_temporal_diff:
                expected_missing.update(
                    'base.temporal_diff_adapter.' + name
                    for name in model.base.temporal_diff_adapter.state_dict()
                )
            if adding_ssa:
                expected_missing.update(
                    'base.ssa.' + name
                    for name in model.base.ssa.state_dict()
                )
            if (
...
def build_optimizer(
    ...
    temporal_diff_multiplier = float(
        getattr(config, 'temporal_frame_temporal_diff_lr_multiplier', 10.0)
    )
    ssa_multiplier = float(
        getattr(config, 'temporal_frame_ssa_lr_multiplier', 10.0)
    )
    if (
        base_multiplier <= 0.0
        ...
        or temporal_diff_multiplier <= 0.0
        or ssa_multiplier <= 0.0
    ):
    ...
    temporal_diff_parameters = []
    if getattr(model.base, 'temporal_diff_enabled', False):
        temporal_diff_parameters = list(model.base.temporal_diff_adapter.parameters())
    temporal_diff_parameter_ids = {id(parameter) for parameter in temporal_diff_parameters}
    ssa_parameters = []
    if getattr(model.base, 'ssa_enabled', False):
        ssa_parameters = list(model.base.ssa.parameters())
    ssa_parameter_ids = {id(parameter) for parameter in ssa_parameters}
    fine_parameters = []
    ...
    m22_freeze_base = bool(
        getattr(config, 'temporal_frame_temporal_diff_freeze_base_enabled', False)
    )
    if m22_freeze_base and (temporal_diff_parameters or ssa_parameters):
        m22_groups = []
        if temporal_diff_parameters:
            m22_groups.append(
                {
                    'name': 'temporal_diff',
                    'params': temporal_diff_parameters,
                    'lr': float(config.lr) * temporal_diff_multiplier,
                }
            )
        if ssa_parameters:
            m22_groups.append(
                {
                    'name': 'ssa',
                    'params': ssa_parameters,
                    'lr': float(config.lr) * ssa_multiplier,
                }
            )
        return optim.AdamW(m22_groups, weight_decay=1e-4)
    ...
    base_parameters = [
        ...
        if (
            id(parameter) not in confidence_parameter_ids
            and id(parameter) not in temporal_diff_parameter_ids
            and id(parameter) not in ssa_parameter_ids
        )
    ]
...
if __name__ == '__main__':
    ...
    temporal_diff_enabled = bool(
        getattr(cfg, 'temporal_frame_temporal_diff_enabled', False)
    )
    temporal_diff_freeze_base = bool(
        getattr(cfg, 'temporal_frame_temporal_diff_freeze_base_enabled', False)
    )
    ssa_enabled = bool(getattr(cfg, 'temporal_frame_ssa_enabled', False))
    ...
    if temporal_diff_enabled:
        if int(cfg.temporal_memory_context_bins) < 5:
            raise ValueError(
                'TEMPORAL_MEMORY.context_bins must be at least 5 for temporal '
                'difference features.'
            )
        if int(cfg.temporal_memory_context_bins) % 2 == 0:
            raise ValueError(
                'TEMPORAL_MEMORY.context_bins must be odd for temporal '
                'difference features.'
            )
    ...
    model = BidirectionalTemporalMemoryNet(
        ...
        temporal_diff_enabled=temporal_diff_enabled,
        ssa_enabled=ssa_enabled,
    ...
    if temporal_diff_freeze_base and (temporal_diff_enabled or ssa_enabled):
        m22_parameter_ids = set()
        if temporal_diff_enabled:
            m22_parameter_ids.update(
                id(parameter)
                for parameter in model.base.temporal_diff_adapter.parameters()
            )
        if ssa_enabled:
            m22_parameter_ids.update(
                id(parameter) for parameter in model.base.ssa.parameters()
            )
        for parameter in model.parameters():
            parameter.requires_grad = id(parameter) in m22_parameter_ids
    elif confidence_only_enabled:
