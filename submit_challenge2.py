...
DATASET_MODE = str(getattr(cfg, "challenge_dataset_mode", "val")).strip().lower()
DEBUG_RAW_DIR = os.environ.get("EVSOD_DEBUG_RAW_DIR", "").strip()

if DATASET_MODE not in {"val", "test"}:
    raise ValueError(
        "TEST.challenge_dataset_mode must be 'val' or 'test', got {!r}".format(
            DATASET_MODE
        )
    )
...
if __name__ == "__main__":
    ...
    print("input split:", DATASET_MODE)
    print("input root:", Path(cfg.root) / DATASET_MODE)
    ...
    dataset = EvUAV(cfg, mode=DATASET_MODE)
    dataset.file_list = sorted(dataset.file_list)
    if not dataset.file_list:
        raise RuntimeError(f"No input files found in: {dataset.root}")
    ...
    if sample_level_inference:
        for video_index in range(len(dataset)):
            ...
            if DEBUG_RAW_DIR:
                debug_dir = Path(DEBUG_RAW_DIR)
                debug_dir.mkdir(parents=True, exist_ok=True)
                source_path = Path(dataset.root) / dataset.file_list[video_index]
                np.save(
                    debug_dir / (source_path.stem + ".npy"),
                    raw_predictions.detach().cpu().numpy().astype(np.float32, copy=False),
                )
